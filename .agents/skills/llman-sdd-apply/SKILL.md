...
2. **判定是否 hard bug**（测试失败原因不明 / 间歇性 flake / 回归且首眼看不穿）：
   - **非 hard bug**（明确的 lint/格式/编译错误/校验失败）：进行最小修复（不扩大范围），先重跑「最小失败复现命令」再重跑全部门禁。
   - **hard bug → 升级 diagnose 子流程（r102）**：
     1. **先建 red-capable 命令**（紧、确定、快、agent 可运行，且能在此 bug 上转红）——一个能驱动真实 bug 路径并断言用户确切症状的命令。**MUST NOT 在无 red-capable 命令前进入假设阶段**（盯着代码猜是本流程要防止的失败）。
     2. 运行并确认转红 → 最小化 repro（逐个剔除输入/调用/配置/数据，只留 load-bearing 部分）。
     3. 生成 **3–5 个排序假设**，每个须可证伪（「若 X 是因，则改 Y 会让 bug 消失」）。
     4. 单变量验证（一次只改一个），找到根因后修复。
     5. 若无正确 seam 写回归测试，记录该架构缺口（交 `llman-sdd-arch-review`）。
