...
1. 针对 **primary sources** 调研问题——官方文档、源码、spec、第一方 API——而非对它们的二手转述。把每个论断追溯到拥有它的源头。
...
5. 若调研揭示需要决策，建议进入 `llman-sdd-explore` 的 grilling 分支。
...
- 变更保持最小化且范围明确。
- 标识符或意图不明确时禁止猜测。
- 只引 primary sources；二手转述仅作线索追溯源头。
