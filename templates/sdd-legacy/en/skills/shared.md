...
- Claude Code command bindings: `.claude/commands/llman-sdd/`
- Codex does not generate llman sdd slash-command/custom-prompt bindings; use `llman-sdd-*` skills.

Common actions:
- `/llman-sdd:new <id|description>` → create `llmanspec/changes/<id>/`
- `/llman-sdd:continue <id>` → create the next artifact
- `/llman-sdd:ff <id>` → create all artifacts quickly
- `/llman-sdd:apply <id>` → implement tasks and update checkboxes
- `/llman-sdd:verify <id>` → verify implementation vs artifacts
- `/llman-sdd:archive <id>` → merge deltas + move to `llmanspec/changes/archive/`

Troubleshooting:
- If Claude `/llman-sdd:*` is not recognized, rerun `llman sdd update-skills --all`.
- If legacy bindings exist in old command directories or old Codex prompts, rerun `llman sdd update-skills` in an interactive terminal to migrate (it requires double confirmation).
<!-- endregion -->
