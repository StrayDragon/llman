...
Use llmanspec and the `llman sdd-legacy` workflow.
...
- `llman sdd-legacy update-skills --all`
...
- Regenerate skills with `llman sdd-legacy update-skills`.

Only use AGENTS.md context injection.

Workflow prompts:
- Use the generated `llman-sdd-*` skills (regenerate with `llman sdd-legacy update-skills`).
...
2. Check current state: `llman sdd-legacy list` and `llman sdd-legacy list --specs`.
...
6. Validate: `llman sdd-legacy validate <change-id> --strict --no-interactive`.
...
## Phase 3: Archive
After deployment:
- Run `llman sdd-legacy archive <change-id>`.
- Use `--skip-specs` for tooling-only changes.
- Validate again: `llman sdd-legacy validate --strict --no-interactive`.
