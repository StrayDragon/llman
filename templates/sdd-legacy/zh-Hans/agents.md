...
请使用 llmanspec 与 `llman sdd-legacy` 工作流。
...
2. 查看现有状态：`llman sdd-legacy list` 与 `llman sdd-legacy list --specs`。
...
6. 校验：`llman sdd-legacy validate <change-id> --strict --no-interactive`。
...
## 阶段 3：归档变更
部署后：
- 运行 `llman sdd-legacy archive <change-id>`。
- 仅工具类变更使用 `--skip-specs`。
- 再次校验：`llman sdd-legacy validate --strict --no-interactive`。
...
保留此托管块，便于 `llman sdd-legacy update` 刷新。
