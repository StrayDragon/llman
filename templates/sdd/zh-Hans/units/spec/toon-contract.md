...
- **BDD-on（Git-native）**：在非默认 feature 分支上编辑 live `.feature` 与 `spec.toon`；用 `llman sdd change attach` 绑定；优先 `change finalize` 单 commit 收尾（或 fallback：归档前 `checkpoint` 再 `change archive`）；`diff` 只读审查/导出。合并前 archive/finalize 仅移动 change 文档——由本地 merge 提升 specs（`git switch <default> && git merge --ff-only <feature>`；push / Hosting PR 可选）。**禁止**编写 `*.feature.delta.toon`（遗留活跃 feature_delta 是迁移阻断项）。没有 solidify 命令。
