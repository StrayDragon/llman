...
<!-- source: OpenSpec templates/en/opsx/apply.md (copied 2026-02-09) -->

Implement tasks for a change in `llmanspec/changes/<id>/`.

**Input**: Optionally specify a change id (e.g., `/opsx:apply add-auth`). If omitted, infer from context; if ambiguous, prompt the user to choose.
...
1. **Select the change**
   ...
   Always announce: "Using change: <id>" and how to override (e.g., `/opsx:apply <other>`).
...
2. **Check prerequisites**
   ...
   If missing, suggest using `/opsx:continue <id>` (or `/opsx:ff <id>`) to create planning artifacts first, then STOP.
...
6. **On completion**
   ...
   - Suggest `/opsx:verify <id>` (optional but recommended)
   - Suggest `/opsx:archive <id>` to archive and update main specs
