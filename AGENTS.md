...
会忽略它们。若确定不再需要，手动删除或保留作文档；solidify 在 BDD-off 时会提示未配置
（见 `llman-sdd-solidify`）。
...
- **改 validate 的 `--check`/`--no-check` 语义** → 同步 `validate-check.feature`（runner
  触发条件、降级行为）。
- **改 solidify 行为**（一致性门禁 / `--write-stubs` / BDD-off no-op）→ 同步 `solidify-mode.feature`
  的断言文本。
- **改 Partitioned 门禁**（`@req`、双写、feature_delta archive）→ 同步 `sdd-bdd-mode-compat`
