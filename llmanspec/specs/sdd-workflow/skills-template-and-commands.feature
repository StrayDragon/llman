# language: zh-CN
# 对应 spec: sdd-workflow r4-r6,r19,r21-r22,r24,r28-r29,r90 — init --update 生成结构化技能与 workflow
# commands；模板用 MiniJinja 单元注入复用；sdd 命令范围；模板版本元信息；skills 符合 SKILL.md
# 规范且不暴露 --force；废弃 llman-sdd-* 在 init --update 时按候选集先删再写。
功能: Skills 生成、模板复用与命令范围
  ...
  场景: skills name 与目录一致
    假如 init --update 写入 llman-sdd-archive/SKILL.md
    当 生成完成
    而且 那么frontmatter name 为 llman-sdd-archive

  场景: skills description 非空
    假如 init --update 生成任意 SKILL.md
    当 生成完成
