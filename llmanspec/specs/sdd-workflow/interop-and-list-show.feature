...
功能: OpenSpec 互转、列表查看与标识符安全
  场景: style 参数缺失报错
    ...
    当 命令执行
    那么返回非零并提示需要 --style openspec

  场景: style 参数非法报错
    假如 用户执行 import --style unknown
    当 命令执行
    那么返回非零并提示仅支持 openspec
  ...
  场景: 默认列出变更排除 archive
    ...
    当 命令执行
    那么输出 changes/ 下的变更目录（排除 archive）

  场景: 列出 specs
    假如 用户执行 llman sdd list --specs
    当 命令执行
    那么输出 specs/ 下的 spec 目录

  场景: show 自动识别与歧义处理
    假如 用户执行 llman sdd show <item-name> 且未指定 --type
    当 命令执行
    那么自动识别 change/spec
    而且若同时匹配则报错并提示使用 --type

  场景: show 非交互提示语
    假如 用户在非交互环境执行 llman sdd show
    当 命令执行
    那么输出 Nothing to show 提示并退出码为 1

  场景: validate 非交互提示语
    假如 用户在非交互环境执行 llman sdd validate
    当 命令执行
    那么输出 Nothing to validate 提示并退出码为 1
