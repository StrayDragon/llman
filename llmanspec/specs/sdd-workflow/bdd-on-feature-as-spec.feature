...
功能: BDD-on（feature-as-spec）模式
  场景: 目录即 spec，validate 自动发现 feature
    ...
    而且 那么validate 自动发现 status.feature 并解析
    而且 而且不要求任何注册表或 spec.toon 引用表

  场景: 新增 feature 无需注册
    假如 config.yaml 含 bdd: 段
    当 用户将 new-behavior.feature 放入 specs/cli/ 目录
    那么 validate 自动发现该文件
    而且 而且无需修改 spec.toon 或任何 manifest

  场景: spec.toon 仅校验元数据
    假如 config.yaml 含 bdd: 段
    当 validate 解析 specs/cli/spec.toon
    而且 那么仅校验 kind/name/purpose
    而且 而且不要求 valid_scope
