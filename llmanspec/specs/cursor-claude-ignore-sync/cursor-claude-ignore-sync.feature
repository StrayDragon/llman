...
功能: cursor-claude-ignore-sync
  ...
  场景: git 守卫、交互式模式与 x 子命令快捷方式
    - 对应 spec: cursor-claude-ignore-sync — 系统 MUST 强制检查 git root（非 git 目录报错， --force 可绕过）；MUST 提供交互式模式选择 targets/预览/确认；SHOULD 通过 llman x 子命令

  @req:r50 @human
  场景: include 规则解析与 Claude Code settings 读写策略
    - 对应 spec: cursor-claude-ignore-sync — 系统 MUST 解析 gitignore 风格的 include（!pattern） 规则并稳定写回；MUST 解析/更新 Claude Code settings（仅 permissions.deny 的 Read(...)），
