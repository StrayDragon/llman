...
功能: 标识规则、分组推断与树形多选
  ...
  场景: agents 为默认 target 之一
    ...
    当 加载默认 targets
    那么含 agents project（.agents/skills）以及 claude user/project
  ...
  场景: 展示 skill_id 与目录名
    ...
    当 交互选项展示
    那么显示为 brainstorming (superpowers.brainstorming)

  场景: 命令行帮助不含预设参数
    假如 用户查看 llman skills --help
    当 检查帮助
    那么不含 --preset、--save-preset、--list-presets 等参数
