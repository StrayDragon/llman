...
功能: stats 命令视图、稳定 id 与过滤
  ...
  场景: 命令帮助可发现
    ...
    当 命令执行
    而且 那么打印 stats 命令帮助文本并成功退出

  @req:r1
  场景: 默认过滤到当前目录
    假如 用户在目录 /p/a 运行 llman x claude-code stats
    当 命令执行
    而且 那么输出排除 cwd 不精确等于 /p/a 的会话
