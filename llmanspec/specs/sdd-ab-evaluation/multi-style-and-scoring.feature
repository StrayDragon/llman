...
功能: multi-style 对比、快照、聚合与可选软评分
  ...
  场景: 报告优先安全与质量指标
    ...
    当 检查报告
    而且 那么含质量与安全评分
    而且 而且排在 token/latency 指标之前

  @req:r25
  场景: agent 读编辑主 spec 文件
    假如 multi-style agentic 评测运行
    当 agent 执行
    而且 那么读取当前 workspace 的主 spec 文件

  @req:r25
  场景: agent 读编辑 delta spec 文件
    假如 评测含某 change
    当 agent 执行
    而且 那么读取该 change 的 delta spec 文件

  @req:r25
  场景: 同一任务在三种风格下跑通
    假如 维护者执行 multi-style 评测
    当 评测运行
    而且 那么为三种风格创建隔离 workspace 并分别运行相同任务

  @req:r25
  场景: runner 评测前预置 baseline
    假如 一次新评测 run 开始
    当 runner 准备
    而且 那么创建隔离 workspace 并预置语义等价的 baseline specs/changes

  @req:r25
  场景: 评测产物含 meta 快照
    假如 一次评测运行结束
    当 检查临时根目录
    而且 那么存在 meta/（或等价）保存快照与日志

  @req:r25
  场景: --runs 产聚合报告
    假如 维护者运行 just sdd-claude-style-eval --runs 10（或等价）
    当 评测完成
    而且 那么runner 写出聚合汇总到 meta/aggregate.md（和/或 .json）
