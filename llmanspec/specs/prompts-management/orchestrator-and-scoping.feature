...
功能: prompts 编排入口、app 隔离与 scope 解析
  场景: 非交互输出迁移提示
    ...
    当 命令执行
    而且 那么输出提示用户使用 llman x cursor/codex/claude-code prompts

  场景: 使用别名调用等价
    假如 用户运行 llman prompt --no-interactive
    当 命令执行
    而且 那么行为与 llman prompts --no-interactive 等价

  场景: app 维度隔离模板
    假如 用户分别对 cursor 与 codex prompts upsert 写入并用相同 name
    当 命令执行
    而且 那么两者模板互不覆盖
    而且 而且cursor prompts list 不显示 codex 模板
  ...
  场景: cursor 传入不支持的 scope 报错
    ...
    当 命令执行
    而且 那么返回错误并提示 cursor 仅支持 project
