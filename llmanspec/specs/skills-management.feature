...
# purpose: 规范 llman 跨来源技能发现、托管快照、冲突处理与目标链接的整体流程和约束。
# scope: llmanspec/specs/skills-management.feature
...
功能: skills-management
  ...
  场景: 交互入口、扫描与 scope 语义
    - The system MUST satisfy the harness scenarios for `交互入口、扫描与 scope 语义`: 对应 spec: skills-management — llman skills 交互式扫描后直接进 agent→scope→skills 流程； 尊重忽略规则并跳过软链接解析；每个 skills.repo 根目录 MUST 仅扫描直子目录一层（`<root>/<skill>/SKILL.md`），MUST NOT 递归导入更深层嵌套技能；取消是安全 no-op； 项目范围菜单隐藏仅 user scope 已管理技能；scope 排序动态化。

  @req:r80 @human
  场景: 链接启用/禁用、冲突处理与实时状态
    - The system MUST satisfy the harness scenarios for `链接启用/禁用、冲突处理与实时状态`: 对应 spec: skills-management — 按 agent 目标启用/禁用技能（link 模式创建软链接，skip 跳过）； 断链 symlink 视为已存在条目；冲突提示取消是安全 abort；非交互冲突需 --target-conflict 策略； 启用状态实时计算不依赖 registry。
