...
- [x] T1 config `change_id:` 段（垂直切片：schema → 解析 → 测试）
...
- [x] T3 validate pattern 门禁 [blocked-by: T1]
...
- [x] T4 `change new --from` 模板渲染 + `--dry-run` [blocked-by: T1, T2]
...
- [x] T5 `change next-id` 子命令 [blocked-by: T2]
...
- [x] T6 BDD 场景绑定与测试收口 [blocked-by: T3, T4, T5]
