...
3. Determine the next artifact to create (in order):
   ...
   5) Edit live `llmanspec/specs/<capability>.feature` (flat, or directory main file) on the **bound branch** and commit — Specs landing (or set `skip_specs_landing: true` when there is no contract edit)
