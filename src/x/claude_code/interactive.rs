...
use inquire::{Select, Editor, Text, Confirm, validator::Validation};
use rust_i18n::t;
...
pub fn prompt_import_config() -> Result<Option<(String, ConfigGroup)>> {
    ...
    let name = Text::new(&t!("claude_code.interactive.config_group_name"))
        .with_validator(|input: &str| {
            if input.trim().is_empty() {
                Ok(Validation::Invalid(t!("claude_code.validation.name_required").into()))
            } else if input.contains(' ') {
                Ok(Validation::Invalid(t!("claude_code.validation.no_spaces").into()))
            } else {
                Ok(Validation::Valid)
            }
        })
    ...
    println!();
    println!("📝 {} - {}!", t!("claude_code.interactive.multi_line_json"), t!("claude_code.interactive.you_can_now_paste"));
    println!();
    ...
    println!("  • {} ({})", t!("claude_code.interactive.paste_json"), t!("claude_code.interactive.supports_multiple_lines"));
    println!("  • {} ({})", t!("claude_code.interactive.navigate"), t!("claude_code.interactive.use_arrow_keys"));
    println!("  • {} ({})", t!("claude_code.interactive.finish"), t!("claude_code.interactive.ctrl_d_or_enter"));
    println!("  • {} ({})", t!("claude_code.interactive.cancel"), t!("claude_code.interactive.esc"));
    println!();
    ...
    match parse_json_config(&json_input) {
        Ok(config_group) => {
            ...
            if Confirm::new(&t!("claude_code.interactive.confirm_import", name = name))
                ...
                .prompt()
                .context("Failed to confirm import")? {
                return Ok(Some((name, config_group)));
            } else {
                println!("{}", t!("claude_code.interactive.import_cancelled"));
                Ok(None)
            }
