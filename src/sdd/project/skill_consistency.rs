...
/// invalid, or mismatches `config`. Non-prefixed custom skills are ignored.
pub fn check_installed_skills_bdd_mode(root: &Path, config: &SddConfig) -> Result<()> {
    ...
    for entry in fs::read_dir(&skills_dir)? {
        ...
        if !skill_md.exists() {
            violations.push((skill_md, "missing SKILL.md".to_string()));
            continue;
        }
        match read_bdd_mode(&skill_md) {
            Ok(Some(mode)) if mode == expected => {}
            Ok(Some(mode)) => {
                violations.push((skill_md, format!("bdd_mode={mode}, expected {expected}")));
            }
            Ok(None) => {
                violations.push((
                    skill_md,
...
fn read_bdd_mode(path: &Path) -> Result<Option<String>, String> {
