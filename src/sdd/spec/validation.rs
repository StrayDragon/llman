...
pub fn validate_spec_content_with_frontmatter_and_bdd(
    ...
    match parse_result {
        Ok(doc) => {
            ...
            // BDD-on: `.feature` files are the executable subset of the spec's
            // scenarios (generated by `solidify`). Validate their Gherkin
            // structure in fast mode and run the configured runner in full mode.
            if bdd_enabled
...
fn validate_main_spec_doc(doc: &MainSpecDoc, spec_name: &str) -> Vec<ValidationIssue> {
    ...
    // A spec MUST declare at least one requirement (with a MUST/SHALL statement).
    // Unified path: applies to BDD-on and BDD-off alike — spec.toon is the SSOT.
    ...
    for (idx, req) in doc.requirements.iter().enumerate() {
        let count = scenarios_by_req
            ...
            .copied()
            .unwrap_or(0);
        // Every requirement must have at least one scenario (in spec.toon, the
        // SSOT; scenarios with feature=false are documentation-only but still
