...
pub static BACKEND: YamlBackend = YamlBackend;

impl SpecBackend for YamlBackend {
    ...
    fn parse_main_spec(&self, content: &str, context: &str) -> Result<MainSpecDoc> {
        ...
        let doc: MainSpecDoc = serde_yaml::from_str(&payload)
            .map_err(|err| anyhow!("{context}: failed to parse YAML payload: {err}"))?;
    ...
    fn parse_delta_spec(&self, content: &str, context: &str) -> Result<DeltaSpecDoc> {
        let payload = extract_single_style_payload(content, self.style(), context)?;
        let doc: DeltaSpecDoc = serde_yaml::from_str(&payload)
            .map_err(|err| anyhow!("{context}: failed to parse YAML payload: {err}"))?;
    ...
    fn dump_main_spec(&self, doc: &MainSpecDoc, options: DumpOptions) -> Result<String> {
        ...
        let emitted = serde_yaml::to_string(doc)
            .map_err(|err| anyhow!("failed to serialize YAML payload: {err}"))?;
        Ok(strip_yaml_document_start(&emitted).trim_end().to_string())
    }
...
fn strip_yaml_document_start(value: &str) -> &str {
