...
应用 YAML LSP schema header 时，工具 MUST 确保文件顶部存在且仅存在一个有效的 schema header，并 MUST NOT 删除不相关的行或内容。

#### Scenario: 存在多条 header 行
- **WHEN** 某 YAML 文件包含多条 `# yaml-language-server: $schema=...` 行
- **THEN** 工具将文件重写为“顶部一条正确 header”，同时保留其余内容不变
