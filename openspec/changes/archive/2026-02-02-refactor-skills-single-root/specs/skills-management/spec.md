...
- FROM: `### Requirement: 可配置来源与目标并提供默认值`
- TO: `### Requirement: 可配置目标并提供默认值`
...
**原因**：不再依赖 repo scope 或 git 根目录，运行位置不应阻断 skills 管理。
**迁移**：无。

### Requirement: 冲突检测与版本选择
**原因**：移除多来源扫描与版本选择，单一来源无需 hash 冲突选择。
**迁移**：将需要的技能手动放入 `<skills_root>`。

### Requirement: 基于内容哈希的快照跟踪
**原因**：移除 `store/versions` 快照机制。
**迁移**：使用 git 作为版本历史。

### Requirement: 自动托管与来源只读
**原因**：不再复制到 `store/`，直接以 `<skills_root>` 作为来源。
**迁移**：把 `store/<skill_id>/current` 复制到 `<skills_root>/<skill_id>`。

### Requirement: 目标复制冲突处理
**原因**：移除 `copy` 模式，仅保留 `link/skip`。
**迁移**：将目标同步策略改为软链接，必要时先清理目标目录。
