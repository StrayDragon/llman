...
- 内置 spec-driven 模板（proposal/spec/design/tasks），保持与 OpenSpec schema 对齐。
- 实现变更/规范发现、校验与归档，使 `openspec/specs` 仍然作为唯一真相来源。
- `llman sdd update` 写入并维护 `openspec/AGENTS.md` 中的 llman SDD 提示块，提醒 agent 在特定场景使用 `llman sdd` 管理 specs。
- `list / show / validate` 支持 `--json` 输出，便于 llman 处理。
...
- 影响的代码：`src/cli.rs`、新增 `src/sdd/**`、`locales/app.yml`、`README.md`、`tests/**`、模板资源。
