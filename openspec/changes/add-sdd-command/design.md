## 背景
llman 需要一个内建的规范驱动开发工作流，但不依赖 Node 版 OpenSpec CLI。目标是镜像 OpenSpec 的核心行为（双目录模型、proposal/spec/design/tasks 工件、校验、归档），同时保持实现小而可控。

## 目标 / 非目标
**目标：**
- 提供原生 `llman sdd` 命令组（init / update / list / show / validate / archive）。
- 保持与 OpenSpec 目录结构兼容（`openspec/`、`openspec/specs`、`openspec/changes`）。
- 内置 spec-driven 模板与校验规则，行为与 OpenSpec schema 对齐。
- `llman sdd update` 写入并维护 `openspec/AGENTS.md` 中的 SDD 提示块，用于在特定场景提醒 agent 使用 `llman sdd`。
- `list / show / validate` 提供 `--json` 输出。
...
- 模板以资源文件形式内置（例如 `templates/sdd/spec-driven/`），由 init/update 写入 `openspec/`。
- `openspec/AGENTS.md` 写入策略：使用受管块（例如 `<!-- LLMAN-SDD:START -->`/`<!-- LLMAN-SDD:END -->`），`update` 仅替换该块并保留用户自定义内容；若文件缺失则生成完整模板。
- `list / show / validate` 默认文本输出，并提供 `--json` 机器可读模式。
...
- `llman sdd init` 为新项目提供 `openspec/` 基线结构。
