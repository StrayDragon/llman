...
- [x] 4.2 Enforce canonical validation rules: `version` (v1), `kind`, `name` (strict), uniqueness constraints, and ≥1 scenario per requirement.
...
- [x] 5.2 Enforce delta validation rules: `version` (v1), `kind`, op/field rules (including `~` null handling and strict scenario rules for add/modify vs remove/rename).
