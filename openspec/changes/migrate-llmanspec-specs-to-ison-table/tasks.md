...
- [ ] 2.2 Keep legacy JSON parsing + JSON repair logic isolated to the `sdd-legacy` command so `llman sdd` can be table/object ISON-only.
...
- [ ] 4.2 Enforce canonical validation rules: `version` (v1), `kind`, `name` (strict), uniqueness constraints, and ≥1 scenario per requirement.
...
- [ ] 5.2 Enforce delta validation rules: `version` (v1), `kind`, op/field rules (including `~` null handling and strict scenario rules for add/modify vs remove/rename).
...
- [ ] 6.1 Implement token-friendly default dumping (no alignment padding) with stable canonical block ordering (`object.*` → main table → scenarios table).
...
- [ ] 6.3 Ensure row ordering is stable (no auto-sorting); writers preserve existing order and append new rows at the end.
- [ ] 6.4 Ensure archive merge writes canonical table/object ISON payloads and preserves deterministic ordering.
